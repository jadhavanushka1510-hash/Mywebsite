...
for row in rows:
    ...
    # Windows path आणि Linux path दोन्ही handle करतो
    stored_path = str(row[7]).replace("\\", "/")
    image_name = stored_path.split("/")[-1]
...
if category == "Electronic Devices":
    ...
    products = [
        p for p in products
        if p["name"].lower() in [x.lower() for x in electronics]
    ]
    ...
    products = [
        p for p in products
        if p["name"].lower() in [x.lower() for x in men]
    ]
    ...
    products = [
        p for p in products
        if p["name"].lower() in [x.lower() for x in women]
    ]
